"""Phase 3 sweep runner for ANN VGG9 baseline (T=0).

Same structure as run_phase3_sweep.py, but:
- Loads ANN VGG9 checkpoint (T=0, no spiking)
- Uses candidates_ann.py (ANN-compatible, skips SNN-specific methods)
- All candidate IDs are IDENTICAL to SNN version for cross-comparison

Usage:
    python phase3/run_phase3_sweep_ann.py --pretrained <ckpt> --data_root <data>
        [--gpu 0] [--level 1,3,5] [--batch 1,4,8,32,128]
        [--seeds 0,1,2,3] [--quick]
"""
...
def parse_args():
    ...
    p.add_argument("--out_dir", type=str, default="outputs/phase3_ann")
...
def run_setting(args, setting, device, logger):
    ...
    if csv_path.exists() and json_path.exists():
        try:
            ...
            if len(existing.get("rows", [])) >= 11:  # 4 baselines + 7 candidates
...
{'='*60}")
    ...
    def eval_method(engine_fn, method_id, method_name, family, is_baseline=False):
        ...
        for corr in CORRUPTIONS:
            try:
                set_seed(seed)
                model, dev = build_ann_model(args.pretrained, device)
    ...
    logger.info("--- B2: MEMO ---")
    # MEMO: reduce n_aug for large batch to avoid OOM
    memo_n_aug = 8
    if batch_size == 1:
        memo_n_aug = 32
    elif batch_size >= 32:
...
def main():
    ...
    # Build all settings, sorted by difficulty (easy first)
    all_settings = []
    for level in levels:
    ...
    # Sort: level ascending, then batch ascending (easy first)
    all_settings.sort(key=lambda s: (s["level"], s["batch_size"], s["seed"]))

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger = setup_logger("phase3_ann", str(Path(args.out_dir) / "logs"), "overview")
    logger.info(f"args: {vars(args)}")
    logger.info(f"Phase 3 ANN: {len(all_settings)} settings, "
                f"{len(ALL_CANDIDATES)} candidates + 4 baselines")
    logger.info(f"Device: {device}")

    if args.dry_run:
        print(f"\
Phase 3 ANN settings ({len(all_settings)} total):")
...
{traceback.format_exc()}")
    ...
    logger.info("\
" + "="*60)
    logger.info("PHASE 3 ANN COMPLETE")
    logger.info("="*60)
