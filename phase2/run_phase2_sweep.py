"""Phase 2 sweep runner: deep dive on c03 (Margin Loss ZO).

Runs all Phase 2 candidates on a single setting (level3, batch=8, 15 corr).
Robust: one candidate failure never aborts the sweep.

Usage:
    python phase2/run_phase2_sweep.py --pretrained <ckpt> --data_root <data>
...
def main():
        ...
        for s in seeds:
            for corr in corruptions:
                ...
                def make_memo(m=model, d=dev):
                    return MEMOEngine(m.to(d), n_aug=16, device=d)
    ...
    logger.info("=" * 60)
    logger.info("PHASE 2 SUMMARY (ranked by mean acc, BASELINES INCLUDED):")
